...
|   查看路径下的内容    | 1. `ls` <br />2. `ls <DIR1> [DIR2] ...`<br />3. `ls -l \| ll`<br/>4. `ls -h`<br/>5. `ls -a`<br/>6. `ls -R`<br/>7. `ls -d`<br />8. `ls -Z` | 1. 查看当前路径下的内容<br />2. 查看指定单个或多个路径下的内容<br />3. 显示详细<br/>4. 显示看得懂的文件大小<br/>5. 显示所有内容（**包含隐藏目录或文件**）<br/>6. 完整显示目录下的所有文件（**递归查看**）<br/>7. 将目录像其它文件一样列出，而不是列出它们的内容<br />8. 显示对应的 SELinux 标签 |
