...
|       注销或退出终端       | `exit`                                                       | ❗在装有桌面环境的主机下执行，只会关闭终端窗口：              |
...
|   查看路径下的内容    | 1. `ls` <br />2. `ls <DIR1> [DIR2] ...`<br />3. `ls -l` / `ll`<br/>4. `ls -h`<br/>5. `ls -a`<br/>6. `ls -R`<br/>7. `ls -d`<br />8. `ls -Z`<br />9. `ls -1` | 1. 查看当前路径下的内容<br />2. 查看指定单个或多个路径下的内容<br />3. 以详细列表形式显示文件内容<br/>4. 显示能看得懂的文件大小<br/>5. 显示所有内容（**包含隐藏目录或文件**）<br/>6. 完整显示目录下的所有文件（**递归查看**）<br/>7. 将目录像其它文件一样列出，而不是列出它们的内容<br />8. 显示对应的 SELinux 标签<br />9. 显示一行一个文件或目录 |
|       新建目录        | 1. `mkdir <dir1> [dir2]...`<br/>2. `-p`                      | 1. 新建单个或多个空目录<br/>2. 递归新建单个或多个空目录      |
|      删除空目录       | 1. `rmdir <dir1> [dir2]...`<br />2. `-p`                     | 1. 删除指定的**空目录**<br />2. 删除父目录及内部的所有**空目录** |
|       新建文件        | `touch <file1> [file2]...`                                   | 新建一个或多个空白的文件                                     |
|     复制文件目录      | 1. `cp <file1> <file2/dir>`<br />2. `-r`<br />3. `-p`<br />4. `-v`<br />5. `-a` | 1. 将文件复制到目标路径<br />2. 递归复制（复制目录）<br />3. 复制时再复制权限<br />4. 显示复制过程<br />5. 此选项通常在复制目录时使用，它保留链接、文件属性，并复制目录下的所有内容。 |
|     删除文件目录      | 1. `rm <file1> [file2]...`<br />2. `-r` / `-R`<br />3. `-f`<br />4. `-rf ` | 1. 删除单个或多个目录或文件，可能需确认<br />2. 递归删除**（包括目录）**<br />3. 强制性删除，**跳过确认**<br />4. 强制性递归删除该目录或文件**（❗ 谨慎操作）** |
| 移动 / 重命名文件目录 | 1. `mv file1 /doc`<br />2. `mv a.file b.txt`                 | 1. 将当前目录下的 `file1` 文件移动到 `/doc` 目录下<br />2. 将当前目录下的 `a.file` 文件重命名为 `b.txt` |
|       文件类型        | `file <file1> [file2]...`                                    | 查看单个或多个文件类型                                       |
|       文件属性        | `stat <file1> [file2]`                                       | 查看单个或多个文件的详细属性                                 |
|   链接（快捷方式）    | 1. `ln <file> <link>`<br />2. ` ln -s <file> <link>`         | 1. 创建一个**硬**链接<br />❗：删除源文件后**仍可用**，**但不能在目录上创建**）<br />2. 创建一个**软**链接<br />❗：删除源文件后**不可用** |
...
|               名称               | 用法 / 选项（简略）                                          | 注释                                                         |
| :------------------------------: | :----------------------------------------------------------- | ------------------------------------------------------------ |
|        显示/编辑文件内容         | 1. `cat <file1> [file2]...`<br />2. `cat -n <file>`<br />2. `cat > <file>`<br />3. `cat >> <file>` | 1. 显示完整的文件内容<br />2. 带行号显示文件里的内容<br />3. 以行编辑模式下编辑文件并将其**覆盖写入**到文件中<br />4. 以行编辑模式下编辑文件并将其**追加写入**到文件中 |
...
|             翻页信息             | 1. `<command> \| more`<br />2. `<command> \| less`<br />3. `more <file>`<br />4. `less <file>` | 1. `more` 仅支持向下翻页，不支持向上翻页<br />2. `less` 都支持向上向下翻页 |
...
|            行筛选工具            | 1. `<command> \| grep <String>`<br />2. `grep <keyword> <file1> [file2] ` | 1. 通过此方法可直接筛选所包含的行并输出<br />2. 在单个或多个文件中查找行内包含的关键字并输出 |
...
|       运行选项       | `init <X>`                                                   | 其中的 `<X>` 表示运行级别，具体如下：<br />- `0` ：关机<br />- `1`：进入紧急模式<br />- `2`：多用户模式<br />- `3`：带网络的多用户模式<br />- `4`：未定义 *没什么用*<br />- `5`：图形界面<br />- `6`：重启 |
